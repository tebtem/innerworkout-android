package com.tech.inner_workout.ui.settings.customer
...
import com.tech.inner_workout.databinding.ActivityCustomerSupportBinding
...
class CustomerSupportActivity : BaseActivity<ActivityCustomerSupportBinding>() {
    private val viewModel: CustomerSupportVM by viewModels()
    ...
    private fun initClick() {
        viewModel.onClick.observe(this) {
            when (it?.id) {
                ...
                R.id.tvFAQ->{
                    val uri = Uri.parse("https://www.innerworkout.co/about")
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode === Activity.RESULT_OK) {
              val   photo = result.data?.extras?.get("data") as Bitmap
                if (photo != null) {
                    /*adapter.setAttachBeans(AttachBean(photo))
                    adapter.setEnableItemRemove(true)*/
                }
            }
        }
